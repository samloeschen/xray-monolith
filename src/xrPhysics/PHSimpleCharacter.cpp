...
{
	Triangle tri;
	CalculateInitTriangle(inl_ph_world().ObjectSpace().GetStaticTris() + tri_id, tri,
	                      inl_ph_world().ObjectSpace().GetStaticVerts());
	...
	for (u8 i = 0; 3 > i; ++i)
	{
		...
		{
			if (_abs(fv_dir.dotproduct(crs)) * box.z + _abs(side_dir.dotproduct(crs)) * box.x < sg_dist * dist) return
				false;
		}
	}
	return true;
...
void CPHSimpleCharacter::TestPathCallback(bool& do_colide, bool bo1, dContact& c, SGameMtl* /*material_1*/,
                                          SGameMtl* /*material_2*/)
{
	do_colide = false;
...
void CPHSimpleCharacter::ApplyImpulse(const Fvector& dir, dReal P)
{
	...
	dBodySetForce(m_body, m_ext_imulse.x * P / fixed_step, m_ext_imulse.y * P / fixed_step,
	              m_ext_imulse.z * P / fixed_step);
}
...
void CPHSimpleCharacter::PhTune(dReal step)
{
	...
	if (b_jumping && b_good_graund || (m_elevator_state.ClimbingState() && b_valide_wall_contact))
		//b_good_graund=b_valide_ground_contact&&m_ground_contact_normal[1]>M_SQRT1_2
		b_jumping = false;
	...
	if (!b_on_ground && !m_elevator_state.ClimbingState())
	{
		...
		dVector3 dif = {
			current_pos[0] - m_depart_position[0],
	...
	if (b_jumping)
	{
		...
		const dReal* current_pos = dBodyGetPosition(m_body);
		dVector3 dif = {
			current_pos[0] - m_jump_depart_position[0],
...
#ifdef DRAW_BOXES
	GLevel().debug_renderer().draw_aabb			(m_bcenter,m_AABB.x,m_AABB.y,m_AABB.z,D3DCOLOR_XRGB(0,0,255));
	GLevel().debug_renderer().draw_aabb			(m_bcenter_forbid,m_AABB_forbid.x,m_AABB_forbid.y,m_AABB_forbid.z,D3DCOLOR_XRGB(255,0,0));
#endif
///M.c.set(0.f,1.f,0.f);
...
u16 CPHSimpleCharacter::RetriveContactBone()
{
	...
	else
	{
		...
		pos_in_object.sub(m_collision_damage_info.HitPos(), m_phys_ref_object->ObjectPosition());
		//vector from object center to contact position currently in global frame
		Fmatrix object_form;
		object_form.set(m_phys_ref_object->ObjectXFORM());
		object_form.transpose();
		object_form.transform_dir(pos_in_object);
		//project pos_in_object on object axes now it is position of contact in object frame
...
void CPHSimpleCharacter::TestRestrictorContactCallbackFun(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1,
                                                          SGameMtl* material_2)
{
	dGeomID g_this = NULL;
	dGeomID g_obj = NULL;
...
void CPHSimpleCharacter::Collide()
{
	...
	inherited::Collide();
	if (injuriousMaterialIDX == GAMEMTL_NONE_IDX && (*p_lastMaterialIDX) != GAMEMTL_NONE_IDX && GMLibrary()
	                                                                                            .GetMaterialByIdx(
		                                                                                            *p_lastMaterialIDX)
	                                                                                            ->Flags.test(
		                                                                                            SGameMtl::
