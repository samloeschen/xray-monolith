...
void CRenderTarget::phase_hdr10_bloom()
{
    ...
    // downsample
    {
        RCache.set_Element(s_hdr10_bloom_downsample->E[0]);
    ...
    // blur
    {
        ...
        for (int pass = 0; pass < ps_r4_hdr10_bloom_blur_passes; pass++) {
            // alternating horizontal and vertical blurs per pass
            for (int dir = 0; dir < 2; dir++) {
                Swap(src, dst);

                RCache.set_Element(s_hdr10_bloom_blur->E[src]);
                RCache.set_Geometry(g_combine);

                float dx = ps_r4_hdr10_bloom_blur_scale * float(pass + 1) * 1.0f / bloom_w;
    ...
    // upsample and blend
    {
        RCache.set_Element(s_hdr10_bloom_upsample->E[dst]);
        RCache.set_Geometry(g_combine);
