...
void CSightAction::execute_position(Fvector const& look_position)
{
	if (m_torso_look)
		object().sight().SetFirePointLookAngles(m_vector3d, object().movement().m_head.target.yaw,
		                                        object().movement().m_head.target.pitch, look_position);
	else
...
void CSightAction::execute_object()
{
	...
	if (m_torso_look)
		object().sight().SetFirePointLookAngles(look_pos, object().movement().m_head.target.yaw,
		                                        object().movement().m_head.target.pitch, my_position);
	else
		object().sight().SetPointLookAngles(look_pos, object().movement().m_head.target.yaw,
		                                    object().movement().m_head.target.pitch, my_position);
