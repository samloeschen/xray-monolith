...
void CPHMovementControl::Calculate(Fvector& vAccel, const Fvector& camDir, float /**ang_speed/**/, float jump, float /**dt/**/, bool /**bLight/**/)
{
	...
	ICollisionHitCallback* hit_callback = cdi ? cdi->HitCallback() : nullptr;
	if (hit_callback)
		hit_callback->call(gcontact_Initiator, fMinCrashSpeed, fMaxCrashSpeed, fContactSpeed, gcontact_HealthLost, cdi);
...
void CPHMovementControl::PHCaptureObject(CPhysicsShellHolder* object, CPHCaptureBoneCallback* cb)
{
	if (m_capture)
		return;
